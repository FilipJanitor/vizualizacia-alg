app.controller('simulationController', ['$scope', '$window', '$log', 'simulationService', function($scope, $window, $log, simulationService) {
    ...
    $scope.$watch('kNumber.value', function() {
        var i;
        for (i = 0; i < $scope.kNumber.value; i++) {
    ...
    $scope.findDeltaAndStartStep = function() {
        ...
        var moving;
        var wasSet = false;
        for (i = 0; i < $scope.deltaFunction.value.length; i++) {
    ...
    $scope.nextStep = function() {
        ...
        var endBlocksArray;
        var i,j,k,counter;
        var lastIndexOfIBlock;
        var lastIndexOfIMinusOneBlock;
        var lastIndexOfMinusIFullBlock;
        var firstIndexOfMinusIFullBlock;
        var lastIndexOfIMinusOneEmptyBlock;
        var lastIndexOfMinusIHalfFullBlock;
        var firstIndexOfMinusIHalfFullBlock;
        ...
        switch (tempContainer.getStepState()) {
            ...
            case $scope.simulationStateEnum.VISUALIZE_EMPTY_BLOCK_REARRANGE_SYMBOLS:
                ...
                iBlockIndex = tempContainer.getIBlockNumber();
                /*prava strana - ten priklad co je v skriptach*/
                if (iBlockIndex > 0) {
                    tempEnd = Math.pow(2, iBlockIndex-1);
            ...
            case $scope.simulationStateEnum.EMPTY_BLOCK_REARRANGE_SYMBOLS:
                ...
                if (iBlockIndex > 0) {
                    ...
                    for (j = 1; j < endBlocksArray.length; j++) {
                        for (k = endBlocksArray[j - 1]; k < endBlocksArray[j]; k++) {
                            if (k === 0) {
                    ...
                    for (j = endBlocksArray.length -1; j > 0 ; j--) {
                        for (k = endBlocksArray[j]+1; k <= endBlocksArray[j-1]; k++) {
                            $scope.reducedMachineCopyTapeArray.push($scope.simulationStorageTapeArray.value[tempContainer.getIndexOfOriginalTrack()].get(k).lowerLevel);
                            $scope.simulationStorageTapeArray.value[tempContainer.getIndexOfOriginalTrack()].get(k).lowerLevel = " ";
                            if($scope.greenCopyPrintingArray.length < 17){
                                $scope.greenCopyPrintingArray.push(placeholder);
            ...
            case $scope.simulationStateEnum.VISUALIZE_EMPTY_BLOCK_FROM_COPY_TAPE:
                ...
                if (iBlockIndex > 0) {
                    lastIndexOfIBlock = Math.pow(2, iBlockIndex);
                    for (i = 1; i < lastIndexOfIBlock; i++) {
                        $scope.greenStoragePrintingArray.push(new PrintingSquare(i,1,tempContainer.getIndexOfOriginalTrack()));
                    }
                /*Lava strana*/
                } else {
                    lastIndexOfIBlock = -Math.pow(2, -iBlockIndex);
                    for (i = lastIndexOfIBlock + 1; i <= -1; i++) {
                        $scope.greenStoragePrintingArray.push(new PrintingSquare(i,1,tempContainer.getIndexOfOriginalTrack()));
                    }
                }
                break;
            ...
            case $scope.simulationStateEnum.VISUALIZE_HALF_EMPTY_BLOCK_FROM_COPY_TAPE:
                ...
                if (iBlockIndex > 0) {
                    lastIndexOfIBlock = Math.pow(2, iBlockIndex);
                    lastIndexOfIMinusOneBlock = Math.pow(2,iBlockIndex-1);
                    for (i = 1; i < lastIndexOfIMinusOneBlock; i++) {
                        $scope.greenStoragePrintingArray.push(new PrintingSquare(i,1,tempContainer.getIndexOfOriginalTrack()));
                    }
                    for (j = lastIndexOfIMinusOneBlock; j < lastIndexOfIBlock; j++) {
                    ...
                    lastIndexOfIBlock = -Math.pow(2, -iBlockIndex);
                    lastIndexOfIMinusOneBlock = -Math.pow(2,-iBlockIndex-1);
                    for (j = lastIndexOfIBlock + 1; j <= lastIndexOfIMinusOneBlock; j++) {
                        $scope.greenStoragePrintingArray.push(new PrintingSquare(j,0,tempContainer.getIndexOfOriginalTrack()));

                    }
                    for (i = lastIndexOfIMinusOneBlock + 1; i <= -1; i++) {
                        $scope.greenStoragePrintingArray.push(new PrintingSquare(i,1,tempContainer.getIndexOfOriginalTrack()));
                    }
                }
                break;
            ...
            case $scope.simulationStateEnum.VISUALIZE_FULL_BLOCK_ON_OPPOSITE_SIDE:
                ...
                iBlockIndex = tempContainer.getIBlockNumber();
                if (iBlockIndex < 0) {
                    lastIndexOfMinusIFullBlock = -Math.pow(2, -iBlockIndex);
                    /*najdeme zaciatok minus iteho */
                    firstIndexOfMinusIFullBlock = -Math.pow(2, -iBlockIndex - 1);
                    for (j = lastIndexOfMinusIFullBlock + 1; j <= firstIndexOfMinusIFullBlock; j++) {
                        $scope.greenStoragePrintingArray.push(new PrintingSquare(j,0,tempContainer.getIndexOfOriginalTrack()));
                    }
                } else {
                    ...
                    /*najdeme zaciatok minus iteho */
                    firstIndexOfMinusIFullBlock = Math.pow(2, iBlockIndex-1);
                    for (j = firstIndexOfMinusIFullBlock; j < lastIndexOfMinusIFullBlock; j++) {
                        $scope.greenStoragePrintingArray.push(new PrintingSquare(j,0,tempContainer.getIndexOfOriginalTrack()));
                    }
                }
                break;
            ...
            case $scope.simulationStateEnum.VISUALIZE_FULL_BLOCK_FROM_COPY_TAPE:
                ...
                iBlockIndex = tempContainer.getIBlockNumber();
                if (iBlockIndex < 0) {
                    lastIndexOfIMinusOneEmptyBlock = -Math.pow(2, -iBlockIndex -1);
                    for (i = lastIndexOfIMinusOneEmptyBlock+1 ; i <= 0; i++) {
                        $scope.greenStoragePrintingArray.push(new PrintingSquare(i,1,tempContainer.getIndexOfOriginalTrack()));
                    }
                } else {
                    lastIndexOfIMinusOneEmptyBlock = Math.pow(2, iBlockIndex -1);
                    for (i = 0; i < lastIndexOfIMinusOneEmptyBlock; i++) {
                        $scope.greenStoragePrintingArray.push(new PrintingSquare(i,1,tempContainer.getIndexOfOriginalTrack()));
                    }
                }
                break;
            ...
            case $scope.simulationStateEnum.VISUALIZE_HALF_FULL_BLOCK_ON_OPPOSITE_SIDE:
                ...
                /*Spracuvanie druhej casti pravej strana - ten priklad co je v skriptach*/
                if (iBlockIndex < 0) {
                    lastIndexOfMinusIHalfFullBlock = -Math.pow(2, -iBlockIndex);
                    /*najdeme zaciatok minus iteho */
                    firstIndexOfMinusIHalfFullBlock = -Math.pow(2, -iBlockIndex-1);
                    for (j = lastIndexOfMinusIHalfFullBlock + 1; j <= firstIndexOfMinusIHalfFullBlock; j++) {
                    ...
                    /*najdeme zaciatok minus iteho */
                    firstIndexOfMinusIHalfFullBlock = Math.pow(2, iBlockIndex-1);
                    for (j = firstIndexOfMinusIHalfFullBlock; j < lastIndexOfMinusIHalfFullBlock; j++) {
                        $scope.greenStoragePrintingArray.push(new PrintingSquare(j,1,tempContainer.getIndexOfOriginalTrack()));
                    }
                }
                break;
    ...
    $scope.mainSimulatingFunction = function(writingArr, movementArr) {
        ...
        for (j = 0; j < $scope.kNumber.value; j++) {
            ...
            if (movementArr[j] == "-1") {
                for (k = 1; k < $scope.simulationStorageTapeArray.value[j].positiveLength(); k++) {
                ...
                if (blockNumber === undefined) { /*narazili sme na koniec pola a je cele plne, vytvorme novy blok na oboch stranach*/
                    poslednyIndex = $scope.simulationStorageTapeArray.value[j].positiveLength() - 1; /*posledny zaplneny index, ktory este mame definovany*/
                    poslednyBlok = Math.floor(Math.log(poslednyIndex) / Math.LN2) + 1;/*posledny blok, co este mame*/
                    poslednyIndexVNovomBloku = Math.pow(2, (poslednyBlok + 2)-1) - 1; /*+2 -1 je tam preto, lebo blok i ma dlzku 2^(i-1)*/
                    for(f = 0; f < $scope.kNumber.value;f++){
                        for (m = poslednyIndex + 1; m <= poslednyIndexVNovomBloku; m++) { /*pridavame symetricky na obe strany. Povodne tam bolo tapeArray.value[j], ale teraz to rpidavame vsetkym*/
            ...
            if (movementArr[j] == "1") {
                for (k = 1; k < $scope.simulationStorageTapeArray.value[j].negativeLength(); k++) {
                    if ($scope.simulationStorageTapeArray.value[j].get(-k).isEmpty() === 1) {
                        blockNumber = -(Math.floor(Math.log(k) / Math.LN2)+1);
                        $scope.currentSimulationStateArray.push(new StepInformationContainer($scope.simulationStateEnum.VISUALIZE_HALF_EMPTY_BLOCK_REARRANGE_SYMBOLS, j, blockNumber, null));
                        $scope.currentSimulationStateArray.push(new StepInformationContainer($scope.simulationStateEnum.HALF_EMPTY_BLOCK_REARRANGE_SYMBOLS, j, blockNumber, null));
                ...
                if (blockNumber === undefined) { /*narazili sme na koniec pola a je cele plne, vytvorme novy blok na oboch stranach*/
                    poslednyIndex = - ($scope.simulationStorageTapeArray.value[j].negativeLength() - 1);
                    poslednyBlok = -(Math.floor(Math.log(-poslednyIndex) / Math.LN2) +1);
                    poslednyIndexVNovomBloku = -(Math.pow(2, (-poslednyBlok + 2) -1) - 1);
                    for(f = 0; f < $scope.kNumber.value;f++){
                        for (m = -poslednyIndex + 1; m <= -poslednyIndexVNovomBloku; m++) {
