app.controller('mainController', ['$scope', 'simulationService', '$window', '$log', function($scope, simulationService, $window, $log) {
    ...
    $scope.setMode = function(mode) {
        var i;
        $scope.kNumber.value = Math.floor($scope.kNumber.value);
        ...
        if (mode == 1) {
            ...
            for (i = 0; i < 3 * $scope.kNumber.value + 1; i++) {
                $scope.masterFormCommas += ",";
            }
    ...
    $scope.startSimulation = function(value) {
        ...
        for (i = 1; i < $scope.kNumber.value; i++) {
        ...
        for (i = 0; i < $scope.kNumber.value; i++) {
            ...
            if (i === 0) {
                for (j = 0; j < $scope.kSourceTapes[0].length-8; j++) {
    ...
    $scope.validateDeltaStartSimulation = function() {
        ...
        for (i = 0; i < $scope.deltaArrayTemp.length; i++) {
        ...
        for (i = 0; i < $scope.deltaFunction.length; i++) {
    ...
    $scope.registerDelta = function(deltaTempRow) {
        ...
        var reading = [];
        for (i = 1; i < 1 + $scope.kNumber.value; i++) {
            if (arr[i].length != 1) {
                deltaInvalid.value = true;
        ...
        if (arr[1 + $scope.kNumber.value].length != 1) {
            ...
            new_state = arr[1 + $scope.kNumber.value];
        }

        var printing = [];
        for (i = $scope.kNumber.value + 2; i < 2 + 2 * $scope.kNumber.value; i++) {
            if (arr[i].length != 1) {
                deltaInvalid.value = true;
        ...
        for (i = 2 + 2 * $scope.kNumber.value; i < 2 + 3 * $scope.kNumber.value; i++) {
